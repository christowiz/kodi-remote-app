...
var Remote = function() {
    ...
    this.onMessage = function(data) {
        console.log('remote, onMessage ' + JSON.stringify(data));
    ...
    this.onMediaListReceived = function(mediaRawData) {
        if (mediaRawData == undefined || mediaRawData == '') return;
        var mediaListJSON = JSON.parse(mediaRawData);
        var mediaList = mediaListJSON.mediaList;
        for (var i = 0; i < mediaList.length; i++) {
            if (
                connectionDetails.host == mediaList[i].host &&
                connectionDetails.port == mediaList[i].port
            ) {
